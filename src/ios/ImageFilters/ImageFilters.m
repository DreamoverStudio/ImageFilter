...
@implementation NGImage (ImageFilters)

@dynamic filter;

- (ImageFilters*)filter {
  ImageFilters *filter = ImageFilters.new;
  filter.image = self;
  return filter;
}
...
- (NGImage *)sharpify {
  return [[self.image bloomWithRadius:1.5f andIntensity:6.f]
          blur:.6f];
}
...
- (NGImage*)toycamera {
  ...
  CIImage *backgroundImage = [CIImage imageWithCGImage:self.image.CGImage];
